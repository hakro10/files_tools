...
export const SeoContent: React.FC<SeoContentProps> = ({ tool }) => {
  return (
    <div className="mt-12 pt-8 border-t border-slate-800/80 space-y-10">
      ...
      <section className="space-y-4">
        ...
        <div className="prose prose-invert max-w-none text-slate-300 text-sm leading-relaxed whitespace-pre-line bg-slate-900/40 p-6 rounded-2xl border border-slate-800/60">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {tool.features.map((feature, index) => (
            ...
            >
              <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-cyan-500/10 text-cyan-400 text-xs font-bold shrink-0 mt-0.5">
                {index + 1}
              </span>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          {tool.privacyFaq.map((faq, idx) => (
            <div
              key={idx}
              className="p-5 rounded-xl bg-slate-900/60 border border-slate-800/80 space-y-2"
            >
