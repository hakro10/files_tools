...
const JsonTreeNode: React.FC<JsonTreeNodeProps> = ({ data, name, isLast = true, searchTerm }) => {
  ...
  const getTypeColor = (val: any) => {
    if (val === null) return 'text-amber-400 font-semibold';
    if (typeof val === 'boolean') return 'text-purple-400 font-semibold';
    if (typeof val === 'number') return 'text-cyan-400 font-mono';
  ...
  if (!isObject) {
    ...
    return (
      <div className="py-0.5 font-mono text-xs flex items-center gap-1.5 hover:bg-slate-800/40 px-1.5 rounded">
        {name && <span className="text-slate-400 font-medium">"{name}":</span>}
  ...
  return (
    <div className="py-0.5 font-mono text-xs">
      <div
        ...
        className="flex items-center gap-1 cursor-pointer hover:bg-slate-800/60 px-1.5 py-0.5 rounded select-none group"
      >
        <span className="text-slate-500 group-hover:text-cyan-400">
          {collapsed ? <ChevronRight className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
        </span>
        {name && <span className="text-slate-300 font-semibold">"{name}": </span>}
        <span className="text-slate-500 font-bold">{isArray ? '[' : '{'}</span>
        {collapsed && (
          <span className="text-slate-500 text-[11px] px-1.5 py-0.2 bg-slate-800 rounded font-sans">
      ...
      {!collapsed && (
        <div className="pl-4 border-l border-slate-800 ml-2 space-y-0.5 my-0.5">
          {keys.map((key, idx) => (
            <JsonTreeNode
              key={key}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col space-y-2">
          ...
          <textarea
            ...
            className="w-full h-[420px] p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-slate-100 placeholder-slate-600 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none resize-none leading-relaxed"
          />
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 disabled:opacity-50 transition"
          ...
          <div className="w-full h-[420px] rounded-xl bg-slate-950 border border-slate-800 overflow-hidden flex flex-col">
            {viewMode === 'tree' ? (
              ...
              <pre className="p-4 overflow-auto text-xs font-mono text-slate-200 h-full leading-relaxed">
