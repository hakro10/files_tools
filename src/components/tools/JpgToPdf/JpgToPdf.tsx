  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((file) => {
      if (!file.type.match(/image\/(jpeg|jpg)/i)) {
        setError('Please upload valid JPG/JPEG image files.');
        return;
      }
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      for (const imgItem of images) {
        const jpgImageBytes = await fetch(imgItem.dataUrl).then((res) => res.arrayBuffer());
        const embeddedImage = await pdfDoc.embedJpg(jpgImageBytes);
      ...
      <div className="p-8 rounded-2xl bg-white dark:bg-slate-950 border-2 border-dashed border-slate-300 dark:border-slate-800 text-center space-y-4 shadow-sm transition-colors">
        ...
        <div className="space-y-1">
          <h3 className="text-base font-bold text-slate-900 dark:text-white">
            Upload JPG Images to Convert to PDF
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Select one or multiple JPG/JPEG files. All processing occurs 100% in your browser.
          </p>
        </div>
            ...
            {images.map((img, idx) => (
              ...
              >
                ...
                <img
                  src={img.dataUrl}
                  alt={img.name}
                  className="w-12 h-12 rounded object-cover border border-slate-300 dark:border-slate-800 shrink-0"
                />
