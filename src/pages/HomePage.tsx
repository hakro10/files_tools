  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
        ...
        <div className="relative p-8 md:p-12 rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900/90 to-slate-950 border border-slate-800 shadow-2xl overflow-hidden text-center space-y-6">
          ...
          <div className="relative z-10 space-y-4 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-4 text-xs font-mono text-slate-300">
              <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center justify-center gap-2">
        ...
        {/* 2. Top Header AdSlot (Moved DIRECTLY BELOW Hero and ABOVE Tools Section per AdSense guidelines) */}
        <AdSlotHeader />
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-cyan-400 font-semibold bg-cyan-500/10 px-3 py-1 rounded-full border border-cyan-500/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {TOOLS.map((tool) => (
              <Link
                key={tool.id}
                to={`/tools/${tool.slug}`}
                className="group relative p-6 rounded-2xl bg-slate-900/60 border border-slate-800 hover:border-cyan-500/50 hover:bg-slate-900 transition-all duration-300 shadow-xl flex flex-col justify-between"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 group-hover:scale-110 transition-transform">
                      {ICON_MAP[tool.iconName] || <Terminal className="w-6 h-6 text-cyan-400" />}
                    </div>
                    {tool.badge && (
                ...
                <div className="pt-4 mt-4 border-t border-slate-800/60 flex items-center justify-between text-xs font-semibold text-cyan-400 group-hover:translate-x-1 transition-transform">
        ...
        <section className="p-8 rounded-3xl bg-slate-900/40 border border-slate-800/80 space-y-8">
